...
function correlation(mps::InfiniteMPS; opr::Dict{String,String}=Dict{String,String}())
    ...
    for it in 1:mps.length, (is1, is2) in eachSiteCombination(mps, it)
        ...
        let (corrIO, prefix) = ssio(merge(nopr, Dict("pair" => "$(tname)$(is1[2])-$(tname)$(is2[2])")), "corr")
            if !isnothing(corrIO)
                print(corrIO, prefix...)
    ...
    if mps.length == 2
        for (is1, is2) in eachSiteCombination(mps, 1, 2)
            let (corrIO, prefix) = ssio(merge(nopr, Dict("pair" => "A$(is1[2])-B$(is2[2])")), "corr")
                if !isnothing(corrIO)
                    print(corrIO, prefix...)
        ...
        for it in 1:mps.length, (is1, is2) in eachSiteCombination(mps, it, it + 1)
            ...
            for (is1, is2) in eachSiteCombination(mps, 1, 2)
                let (corrIO, prefix) = ssio(merge(nopr, Dict("pair" => "$(tname1)$(is1[2])-$(tname2)$(is2[2])")), "corr")
                    if !isnothing(corrIO)
                        print(corrIO, prefix...)
